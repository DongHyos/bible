...
public class VerseSearchDto {
     ...
     */
    public static VerseSearchDto of(String keyword, List<BibleVerse> bibleVerses) {
        ...
        Map<String, Integer> bookCountMap = verseDtos.stream()
                .collect(Collectors.groupingBy(
                        VerseQueryDto::getBookName,
                        Collectors.collectingAndThen(Collectors.counting(), Math::toIntExact)
                ));

        return new VerseSearchDto(
                keyword.trim(),
                verseDtos.size(),
                verseDtos,
                bookCountMap
    ...
    public String toString() {
        return String.format("VerseSearchDto{keyword='%s', totalCount=%d, books=%d}",
