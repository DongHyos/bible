...
@Repository
public interface BibleCategoryRepository extends JpaRepository<BibleCategory, Integer> {
    ...
    // 분류별 책 수 포함 조회
    @Query("SELECT c, COUNT(b) as bookCount FROM BibleCategory c LEFT JOIN c.books b GROUP BY c ORDER BY c.categoryOrder")
