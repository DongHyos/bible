...
@Repository
public interface SermonVerseRepository extends JpaRepository<SermonVerse, Long> {

    // 특정 설교의 모든 구절 조회 (명시적 쿼리 사용)
    @Query("SELECT sv FROM SermonVerse sv " +
    ...
    List<SermonVerse> findBySermonIdWithBook(@Param("sermonId") Long sermonId);
