...
@Repository
public interface SermonJpaRepository extends JpaRepository<SermonEntity, Long> {

    // 특정 구절과 관련된 설교 조회
    @Query("SELECT s FROM SermonEntity s " +
           "WHERE s.id IN (" +
           "    SELECT DISTINCT sv.sermon.id FROM SermonVerseEntity sv " +
    ...
    List<SermonEntity> findByVerse(@Param("bookId") Integer bookId, 
    ...
    // 제목으로 설교 검색
    List<SermonEntity> findByTitleContainingOrderByViewCountDesc(String title);
